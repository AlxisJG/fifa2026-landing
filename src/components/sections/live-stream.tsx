...
import { CompanionAdSlot } from "@/components/ads/companion-ad-slot";
import { BRANDING } from "@/lib/branding";

export function LiveStreamSection() {
  return (
    <section id="live" className="section-shell py-16 sm:py-24">
      <SectionTitle kicker="Streaming" title="Transmision principal" subtitle="Shell premium preparado para JW Player, DRM y beacons de analytics/ad tech." />
      <div className="theater-dark grid gap-5 xl:grid-cols-12">
        <Reveal className="xl:col-span-8">
          <div className="glass-heavy relative overflow-hidden rounded-3xl border border-slate-700/60 bg-[#090f1f] p-4 shadow-[0_25px_80px_rgba(2,6,23,0.55)] sm:p-5">
            ...
            <div className="relative mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-red-500/20 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-red-300">Live</span>
              </div>
            ...
            <div className="relative h-[250px] overflow-hidden rounded-2xl border border-white/15 bg-gradient-to-br from-[#0c1c3d] via-[#0a1225] to-[#070b16] sm:h-[420px]">
              ...
              <div className="relative grid h-full place-items-center">
                <p className="text-center text-sm leading-relaxed text-white/70">JW Player Embed Placeholder<br />Tokenized AWS DRM stream + live SSAI hooks</p>
              </div>
            </div>
            ...
            <div className="relative mt-4 grid gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
              ...
              <div className="flex flex-wrap gap-2">
                {['Auto', '1080p', 'EN', 'Fullscreen'].map((btn) => (
                  <button key={btn} className="rounded-full border border-slate-300 bg-white px-4 py-2 text-[11px] uppercase tracking-[0.15em] text-blue-900 transition hover:bg-slate-100">
                    {btn}
                  </button>
                ))}
              </div>
            ...
            <div className="mt-4">
              <CompanionAdSlot
                title="Live Companion Sponsor"
                sizeLabel="300x250 / 336x280"
                placement="Player Companion"
                variant="player"
              />
            </div>
          </div>
        ...
        <Reveal className="xl:col-span-4" delay={0.12}>
          <aside className="glass-heavy rounded-3xl border border-slate-300 bg-white/95 p-5 shadow-[0_20px_60px_rgba(2,6,23,0.12)] sm:p-6">
